...
class DetailNewsView(DetailView):
    ...
    #     object = super(DetailNewsView, self).get_object(queryset)
    #     object.stars_on_count = object.rating
    #     object.stars_off_count = 5 - object.rating
    #     object.published_date = object.published_date.strftime('%d.%m.%Y')
    #
    #     return object
    ...
    def get_context_data(self, **kwargs):
        ...
        context['category_groups'] = CategoryGroup.objects.all()
        context['news_side'] = News.objects.filter(visibility=1, status=1)
        ...
        context['recs_news'] = News.objects.filter(categories__in=self.object.categories.all(), visibility=1, status=1)[:10]
        if context['recs_news'].count() < 10:
            additional_items = News.objects.filter(visibility=1, status=1).exclude(categories__in=self.object.categories.all())[:10-context['recs_news'].count()]
